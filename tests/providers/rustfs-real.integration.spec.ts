...
const scope = process.env["CASHMEMO_VERIFY_RUSTFS_SCOPE"] ?? "all";
...
describe.skipIf(!enabled)("pinned RustFS real-service semantics", () => {
  it.skipIf(scope === "secondary")(
    "proves Primary put/head/get/version enumeration/every-version delete",
    async () => {
  ...
  it.skipIf(scope === "primary")(
    "proves Secondary conditional durability, read verification, and privileged removal",
    async () => {
      ...
      await expect(ledger.ensureDurable(record)).resolves.toMatchObject({ verifiedDurable: true });
      const loaded = await ledger.loadForCleanup(
