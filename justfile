...
contracts:
  @node scripts/contracts/validate-openapi.mjs
...
test-integration-real:
  @cargo test --manifest-path tests/integration-appwrite/Cargo.toml

test-privacy-real:
  @npm exec -- tsx tests/privacy/scan_captures.ts --self-test

stack-health:
  @docker compose -f infra/compose/docker-compose.yml ps --status running

appwrite-provision:
  @npm exec -- tsx infra/appwrite/provision.ts

appwrite-contract:
  @cargo test --manifest-path tests/integration-appwrite/Cargo.toml schema_contract
...
_acceptance-us1-create:
  ...
  @cargo test --manifest-path tests/integration-appwrite/Cargo.toml --test create_idempotency --test auth_isolation --test not_found_isolation --test direct_tablesdb_denial
  @npm exec -- tsx tests/privacy/scan_captures.ts --self-test
  @node --test tests/privacy/telemetry_allowlist.test.mjs
  @npm exec -- playwright test tests/acceptance/us1_create.spec.ts
