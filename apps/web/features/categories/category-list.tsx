...
export function CategoryList() {
  ...
  async function deleteCategory(category: CategoryContract) {
    ...
    catch (error) { setDeleting(undefined); const statusCode = (error as { response?: { status?: number } }).response?.status; const message = statusCode === 409 ? `Category cannot be deleted while it has references. ${errorText(error)}` : `Could not delete category. ${errorText(error)}`; setDeleteErrors((current) => ({ ...current, [category.id]: message })); }
  }

  function renderCategoryList() {
    if (list.length === 0) return <div className="empty-state"><h2>No {showArchived ? kind : `active ${kind}`} categories</h2><p>{showArchived ? "Create a category to organize entries." : "Archived categories are hidden. Turn on Show archived to manage them."}</p><Button type="button" onClick={() => { setEditing(undefined); setFormOpen(true); }}>Create category</Button></div>;
    return <div className="card-list">{list.map((category) => {
      const archived = Boolean(category.archived_at);
      return <article className={`management-card management-row ${archived ? "is-archived" : ""}`} key={category.id}><div><h2>{category.name}</h2><p className="muted"><span className="status-label">{archived ? "Archived" : "Active"}</span> · {category.kind === "income" ? "Income" : "Expense"}</p>{deleteErrors[category.id] ? <p role="alert" className="field-error">{deleteErrors[category.id]}</p> : null}</div><DropdownMenu><DropdownMenuTrigger render={<Button type="button" variant="quiet" aria-label={`Actions for ${category.name}`}>Actions</Button>} /><DropdownMenuContent align="end"><DropdownMenuItem onClick={() => { setEditing(category); setFormOpen(true); }}>Rename</DropdownMenuItem>{archived ? <DropdownMenuItem onClick={() => void restoreCategory(category)} disabled={restore.isPending}>Restore</DropdownMenuItem> : <DropdownMenuItem onClick={() => { setArchiveError(undefined); setArchiving(category); }}>Archive</DropdownMenuItem>}<DropdownMenuSeparator /><DropdownMenuItem variant="destructive" onClick={() => setDeleting(category)}>Delete forever</DropdownMenuItem></DropdownMenuContent></DropdownMenu></article>;
    })}</div>;
  }
  ...
  return <section className="management-page">
    ...
    <Tabs value={kind} onValueChange={(value) => setKind(value as "expense" | "income")}><TabsList aria-label="Category kind"><TabsTrigger value="expense">Expense</TabsTrigger><TabsTrigger value="income">Income</TabsTrigger></TabsList>
      ...
      <TabsContent value="expense" aria-label="Expense">{kind === "expense" ? renderCategoryList() : null}</TabsContent>
      <TabsContent value="income" aria-label="Income">{kind === "income" ? renderCategoryList() : null}</TabsContent>
    </Tabs>
    {status ? <p role={status.kind === "error" ? "alert" : "status"} className={status.kind === "error" ? "field-error" : "success"}>{status.text}</p> : null}
    <Dialog open={formOpen} onOpenChange={setFormOpen}><DialogContent className="management-dialog"><DialogHeader><DialogTitle>{editing ? "Rename category" : "Create category"}</DialogTitle><DialogDescription>{editing ? "Renaming changes how this label appears across history." : "Income and expense categories stay separate."}</DialogDescription></DialogHeader><CategoryForm category={editing} defaultKind={kind} embedded showHeading={false} onCancel={() => setFormOpen(false)} onSuccess={() => { setFormOpen(false); setEditing(undefined); void invalidateCategory(); }} /></DialogContent></Dialog>
    <Dialog open={Boolean(archiving)} onOpenChange={(open) => { if (!open) { setArchiving(undefined); setArchiveError(undefined); } }}><DialogContent><DialogHeader><DialogTitle>Archive {archiving?.name}?</DialogTitle><DialogDescription>Archiving keeps history, removes this category from new entries, and pauses dependent recurring rules. Restore will not resume those rules.</DialogDescription></DialogHeader>{archiveError ? <p role="alert" className="field-error">Could not archive category. {archiveError}</p> : null}<div className="dialog-actions"><Button type="button" variant="quiet" onClick={() => setArchiving(undefined)}>Cancel</Button><Button type="button" variant="danger" onClick={() => archiving && void archiveCategory(archiving)} disabled={archive.isPending}>{archive.isPending ? "Archiving…" : "Archive category"}</Button></div></DialogContent></Dialog>
