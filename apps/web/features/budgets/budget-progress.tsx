...
export function BudgetProgress({
  ...
  return (
    <article className={`budget-card ${overBudget ? "is-over-budget" : ""}`}>
      <h2>{categoryName}</h2>
      <p className="status-label">{overBudget ? "Over budget" : "Within budget"}</p>
      ...
      <dl className="budget-values">
        ...
        <div>
          <dt>Spent</dt>
          <dd>
            <Amount currency={budget.currency} value={budget.spent} />
          </dd>
        </div>
        <div>
          <dt>Remaining</dt>
          <dd>
            <Amount currency={budget.currency} value={budget.remaining} />
          </dd>
        </div>
      </dl>
    </article>
