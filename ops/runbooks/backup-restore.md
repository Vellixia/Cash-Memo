...
Status: production-safe procedure definition. Repository contracts do not prove recovery. Real
isolated pgBackRest target-time PITR, measured RPO/RTO, and SC-021 closure remain T268.

## Trigger, ownership, and approval

Use for incident recovery, approved continuity drill, or reviewed infrastructure change. Data
Operations owns execution; SRE owns isolation and Dokploy/PostgreSQL/pgBackRest controls;
Security/Privacy owns release review. Require two-role approval before restore creation and before
controlled release. Never use production customer identifiers in tickets or evidence.

## Restore procedure

1. Record approved reason, immutable build/config versions, synthetic procedure ID, and owners.
2. Select pgBackRest target time within the approved backup/WAL recovery window. Record measured
   RPO/RTO only during real T268 execution; never estimate them into evidence.
...
4. Enforce network isolation: no application ingress, API/worker, outbound provider access, or
   customer session path.
5. Confirm the selected backup set, required WAL, repository object versions, and restore copy
   appear in authoritative backup-lineage inventory. Inventory unavailable, stale, incomplete, or
   unverifiable means STOP.
...
9. Inspect actual restored PostgreSQL state at the target time. If authorized controlled use is
   needed, obtain second release approval after verification. Otherwise destroy immediately.
...
measurements from real drill. Forbidden: raw IDs, tokens, keys, financial content, backup/copy names
