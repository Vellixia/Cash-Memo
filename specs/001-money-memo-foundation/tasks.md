...
- [ ] T006 [P] Define runtime configuration names, types, required/secret flags, and fail-closed validation without example secret values in `config/env.schema.json` — reopened by 2026-08-01 post-implementation audit; entropy allowlists and isolated Appwrite gate vars required.
- [ ] T007 [P] Create CI workflow enforcing gate order, contract/schema checks, secret scan, architecture scan, and artifact retention in `.github/workflows/ci.yml` — reopened by 2026-08-01 post-implementation audit; clean Appwrite gate must be reproduced in CI.
...
- [ ] T014 Add committed-secret, high-entropy, forbidden-example, and runtime-key separation scans to the quality gate in `scripts/secrets/scan.sh` — reopened by 2026-08-01 post-implementation audit; entropy analysis and narrow allowlists required.
...
- [ ] T016 Implement the authoritative OpenAPI 3.1 and export JSON Schema 2020-12 validation suite, including canonical time/offset rules, required conditional `purgeDeadline`, generated/verified scale-specific decimal branches from the currency contract, breaking-change, MIME, trailing-newline, and `additionalProperties` checks, in `scripts/contracts/validate-openapi.mjs` and `scripts/contracts/validate-export-schema.mjs` — reopened by 2026-08-01 post-implementation audit; breaking-change validation must require a supplied baseline.
...
- [ ] T018 Create Axum HTTP adapter crate and generate or hand-bind reviewed Rust DTOs matching OpenAPI canonical time, result-set version, purge-deadline, and privacy-error contracts exactly in `backend/crates/http-adapter/Cargo.toml` and `backend/crates/http-adapter/src/contracts/generated.rs` — reopened by 2026-08-01 post-implementation audit; DTOs must match OpenAPI exactly and drift checks must cover Rust.
...
- [ ] T029 Implement audited mapping between HTTP DTOs and domain types with unknown/immutable field rejection, canonical time/offset conversion, lifecycle-conditional purge deadline, result-set version, and safe privacy-error mapping in `backend/crates/http-adapter/src/contracts/mapping.rs` — reopened by 2026-08-01 post-implementation audit; mapping must match corrected Rust DTOs.
...
- [ ] T033 [P] Write lifecycle state-machine tests including exact deadline boundary and forbidden transitions in `backend/crates/domain/tests/lifecycle_properties.rs` — reopened by 2026-08-01 post-implementation audit; missing test coverage must be implemented.
- [ ] T034 [P] Write stable error-code, HTTP mapping, safe detector-ID allowlist, canonical time, conditional purge-deadline, and value/derivative-redaction contract tests in `backend/crates/http-adapter/tests/error_contract.rs` — reopened by 2026-08-01 post-implementation audit; redaction must be proven across Debug/Display/panic/trace/log/crash/diagnostic.
...
- [ ] T041 Create the supported-API Appwrite adapter boundary and REST/GraphQL client with `ttl=0`, transaction support, POST-body queries, bounded retries, and redacted errors in `backend/crates/appwrite-adapter/Cargo.toml` and `backend/crates/appwrite-adapter/src/client.rs` — reopened by 2026-08-01 post-implementation audit; blind retries for mutations and transaction-staging requests must be disabled and response-loss ambiguity tests added.
...
- [ ] T049 Implement the web authentication/server-state foundation with account-switch cleanup, non-persisted TanStack Query provider, and typed credentialed API client with no owner or URL search values in `apps/web/src/lib/auth/session.ts`, `apps/web/src/lib/query/query-provider.tsx`, and `apps/web/src/lib/api/client.ts` — reopened by 2026-08-01 post-implementation audit; active account state must hydrate from the real Appwrite authenticated session and drafts must clear/isolate synchronously on identity change.
...
- [ ] T056 Run the authentication/isolation gate plus shared privacy-canary scanner self-test and store zero-disclosure evidence in `docs/evidence/001/phase-03-auth-isolation.md` — reopened by 2026-08-01 post-implementation audit; real privacy evidence across every required channel and mandatory evidence metadata/artifact links required.
...
- [ ] T060 [P] [US1] Write failing real Appwrite concurrent same-ID/different-ID retry tests including current lifecycle/revision return without write in `tests/integration-appwrite/create_idempotency.rs` — reopened by 2026-08-01 post-implementation audit; response-loss and mutation ambiguity failure-injection tests required.
- [ ] T061 [P] [US1] Write failing OpenAPI create/currency endpoint response, HTTP-422 `PRIVACY_INPUT_REJECTED` with safe B1–B9 ID only, candidate/normalized/derivative exclusion, W1–W3 warning-only acceptance, canonical time/offset, conditional purge deadline, validation, conflict, unavailable, and no-store contract tests in `backend/crates/http-adapter/tests/create_contract.rs` — reopened by 2026-08-01 post-implementation audit; missing test coverage must be implemented.
- [ ] T062 [P] [US1] Write failing Dexie partition, stable UUID, reload/retry, success/discard deletion, and account-switch tests in `apps/web/tests/compose-draft.test.ts` — reopened by 2026-08-01 post-implementation audit; browser reopen, out-of-order write, and hook-level account-switch tests required.
- [ ] T063 [P] [US1] Write failing client Pattern Set B1–B9/W1–W3, adjacent warning, edit/remove/continue, no-network block, and byte-exact input-preservation tests in `apps/web/tests/privacy-pattern-create.test.tsx` — reopened by 2026-08-01 post-implementation audit; complete Pattern Set fixture coverage required.
...
- [ ] T066 [US1] Implement deterministic server Pattern Set v1 preprocessing and B1–B9/W1–W3 decisions with ephemeral buffers only in `backend/crates/domain/src/privacy_pattern_v1.rs` — reopened by 2026-08-01 post-implementation audit; B8 must match research.md exactly and the complete shared Pattern Set registry must be validated, not version only.
- [ ] T067 [US1] Implement behavior-equivalent client Pattern Set v1 detector without analytics or retained derivatives in `apps/web/src/lib/privacy/pattern-set-v1.ts` — reopened by 2026-08-01 post-implementation audit; B8 must match research.md exactly.
...
- [ ] T070 [US1] Implement Appwrite create transaction covering export fence/state, base result-generation invalidation, memo, Category/Money Space counts, and unique `(owner_id, creation_id)` race resolution in `backend/crates/appwrite-adapter/src/create_money_memo.rs` — reopened by 2026-08-01 post-implementation audit; existing-creation transaction path must be explicitly finished with rollback.
...
- [ ] T074 [US1] Implement the compose client-state slice: user-partitioned Dexie draft/retry schema, non-persisted Zustand UI state, stable compose UUID, byte-exact autosave, legitimate retry reuse, discard, and success cleanup in `apps/web/src/lib/compose/db.ts`, `apps/web/src/features/money-memos/compose-store.ts`, and `apps/web/src/features/money-memos/use-compose-session.ts` — reopened by 2026-08-01 post-implementation audit; autosave must be serialized or monotonic latest-write-wins and drafts must clear/isolate synchronously on account identity change.
...
- [ ] T078 [US1] Assemble authenticated create route with synthetic-safe empty/loading/error states in `apps/web/src/app/(app)/money-memos/new/page.tsx` — reopened by 2026-08-01 post-implementation audit; protected production route must run through real authentication.
- [ ] T079 [US1] Run `just acceptance us1-create` and record commands, environment, requirement mapping, privacy scan, and pass/fail evidence in `docs/evidence/001/us1/acceptance.md` — reopened by 2026-08-01 post-implementation audit; prior PASS invalidated. T079 may be checked only when critical findings = 0, high findings = 0, unsupported checked tasks = 0, production authentication works, real Appwrite integration passes, evidence metadata is complete, and T080+ remains untouched.
