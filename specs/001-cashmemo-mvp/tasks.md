...
- [ ] T004 [P] Define fail-closed canonical runtime configuration for separate runtime/identity PostgreSQL connections, RustFS Primary/Secondary, pgBackRest, deletion ledger, OTLP, OpenAI, and Mailpit/Cloudflare email in `.env.example` and `apps/server/src/bootstrap/environment.schema.ts` (FR-083) — REOPENED — self-hosted service architecture reconciliation
- [ ] T005 [P] Provide development bootstrap guidance compatible with the preserved external PostgreSQL, Mailpit/contract providers, future RustFS services, and shared OTLP without local Docker or duplicate shared services in `infra/containers/compose.yaml` and `scripts/local/bootstrap-https.mjs` — REOPENED — self-hosted service architecture reconciliation
...
- [ ] T152 [P] [US5] Define executable failure matrix for STT, AI, network, DB, auth, Cloudflare email, RustFS Primary/Secondary, pgBackRest, telemetry, worker, and calculation fault points in `tests/failure/failure-matrix.ts` (FR-031, FR-111) — REOPENED — self-hosted service architecture reconciliation
...
- [ ] T170 [US8] Test controlled-clock cancel/expiry/inaccessibility and every RustFS object version/delete-marker removal within 24 hours, including retry and ambiguous response behavior, in `tests/operations/export-lifecycle.spec.ts` (FR-094, FR-103; SC-020) — REOPENED — self-hosted service architecture reconciliation
...
- [ ] T192 Implement verifier-controlled suppression cleanup over the self-hosted lineage model, retaining/alerting/retrying on every failed, stale, incomplete, unavailable, or unverifiable proof in `apps/server/src/modules/deletion/suppression-cleanup.service.ts` (FR-100) — REOPENED — self-hosted service architecture reconciliation
- [ ] T193 Add deployment/static policy forbidding untracked local/manual/operator/volume copies and unregistered replicas or restore copies, and requiring an independent production backup failure domain, in `infra/dokploy/policies/backup-copy-policy.json` (FR-100) — REOPENED — self-hosted service architecture reconciliation
- [ ] T194 [P] Write pgBackRest isolated-restore tests for account-first then memo-token matching across key versions, expired sweepers, session revocation, neighboring-data survival, and release denial in `tests/operations/restore-reconciliation.spec.ts` (FR-028, FR-100, FR-115; SC-021) — REOPENED — self-hosted service architecture reconciliation
- [ ] T195 Implement isolated pre-network pgBackRest PITR reconciliation command with exact account/memo suppression and mandatory verification checklist in `scripts/operations/restore-reconcile.mjs` (FR-100, FR-115) — REOPENED — self-hosted service architecture reconciliation
- [ ] T196 Implement registered pgBackRest restore-copy lifecycle, lineage inventory, network-isolation guard, and verified destruction command in `scripts/operations/restore-copy-lifecycle.mjs` (FR-100) — REOPENED — self-hosted service architecture reconciliation
- [ ] T197 Create production-safe pgBackRest PITR restore/release runbook with independent failure-domain requirement, owners, approvals, RPO/RTO, and content-safe evidence in `ops/runbooks/backup-restore.md` (FR-114, FR-115) — REOPENED — self-hosted service architecture reconciliation
- [ ] T198 Create self-hosted suppression cleanup/key rotation/copy-policy-drift runbook with blocker retention and escalation in `ops/runbooks/deletion-suppression-cleanup.md` (FR-100) — REOPENED — self-hosted service architecture reconciliation
...
- [ ] T200 Define quarterly isolated pgBackRest restore and cleanup drill automation, evidence schema, real restored-state checks, and pass/fail rules in `ops/drills/quarterly-restore.ts` and `ops/drills/suppression-cleanup.ts` (FR-114, FR-115; SC-021) — REOPENED — self-hosted service architecture reconciliation
...
- [ ] T219 [P] Define the reviewed self-hosted service/version contract, immutable input policy, Dokploy environment template, and drift-check schema in `infra/dokploy/service-versions.json`, `infra/dokploy/environment.example`, and `infra/dokploy/README.md` — REOPENED — self-hosted service architecture reconciliation
- [ ] T220 [P] Define private Dokploy-network attachment, no-public-object-store posture, API-only ingress boundary, controlled egress requirements, and no duplicate shared services in `infra/dokploy/compose.yaml` and `infra/dokploy/network-policy.json` — REOPENED — self-hosted service architecture reconciliation
- [ ] T221 [P] Define one immutable Cashmemo OCI image digest serving separate API/worker roles, non-root/read-only runtime, health checks, and safe restart/rollback behavior in `infra/dokploy/compose.yaml` (FR-116) — REOPENED — self-hosted service architecture reconciliation
- [ ] T222 [P] Preserve the existing private PostgreSQL 18 service and volume, add non-destructive health/restart configuration guidance, and configure pgBackRest backup/WAL/PITR without inventing resource limits in `infra/dokploy/postgres-preservation.md` and `infra/pgbackrest/` (FR-100, FR-114) — REOPENED — self-hosted service architecture reconciliation
- [ ] T223 [P] Define private versioned RustFS Primary export/evidence buckets and distinct RustFS Secondary pgBackRest/deletion-ledger namespaces, encryption requirements, zero raw-audio storage, and no suppression TTL in `infra/dokploy/compose.yaml` and `infra/dokploy/storage-policy.json` (FR-094, FR-100) — REOPENED — self-hosted service architecture reconciliation
- [ ] T224 [P] Configure Mailpit development and Cloudflare Email Service production delivery selection with content-free status mapping and no external configuration mutation in `infra/dokploy/environment.example` and the email adapters — REOPENED — self-hosted service architecture reconciliation
- [ ] T225 [P] Define existing-Infisical runtime secret injection and separated runtime/worker/migration/restore/deploy/break-glass capabilities without deploying another secret service in `infra/dokploy/secrets-policy.md` (FR-083) — REOPENED — self-hosted service architecture reconciliation
- [ ] T226 [P] Bind Cashmemo OTLP export to the existing shared collector/OpenObserve using allowlist-only telemetry, without deploying duplicate observability services, in `infra/dokploy/observability.md` (FR-110, FR-112) — REOPENED — self-hosted service architecture reconciliation
- [ ] T227 Integrate pgBackRest backup/PITR, copy-policy drift, inventory-failure, missed-drill, RPO/RTO alert contracts, and independent-failure-domain production checks in `infra/dokploy/backup-alert-policy.json` (FR-100, FR-114) — REOPENED — self-hosted service architecture reconciliation
- [ ] T228 Implement a Dokploy-compatible one-shot migration role using the existing advisory lock/schema compatibility/evidence command in `infra/dokploy/compose.migration.yaml` and `scripts/db/migrate-production.mjs` (FR-116) — REOPENED — self-hosted service architecture reconciliation
- [ ] T229 Harden the shared API/worker OCI image with pinned base digest inputs, minimal packages/capabilities, non-root/read-only runtime, role-specific health probes, and SBOM attachment in `infra/containers/Dockerfile` and `infra/containers/docker-bake.hcl` (FR-116) — REOPENED — self-hosted service architecture reconciliation
- [ ] T230 Implement GitHub Actions build/test/scan/SBOM/publish workflow producing an immutable digest and protected Dokploy handoff artifact; deployment remains a separate approved pass in `.github/workflows/deploy.yml` (FR-116, FR-117) — REOPENED — self-hosted service architecture reconciliation
- [ ] T231 Implement and rehearse Dokploy rollback plus expand/contract safe-forward decision paths using the immutable image digest in `scripts/deploy/rollback-or-safe-forward.mjs` and `ops/runbooks/deployment-rollback.md` (FR-116) — REOPENED — self-hosted service architecture reconciliation
...
- [ ] T240 Create and rehearse core journal, STT/AI, Cloudflare email, PostgreSQL/pgBackRest, RustFS, shared telemetry, and provider-config-drift outage runbooks in `ops/runbooks/core-journal-outage.md`, `ops/runbooks/provider-outages.md`, and `ops/runbooks/self-hosted-service-outages.md` (FR-113) — REOPENED — self-hosted service architecture reconciliation
- [ ] T241 Create and rehearse injected-secrets, migration, Dokploy deployment, audio, lifecycle backlog, export exposure, RustFS/backup, rate-abuse, and cross-user incident runbooks in `ops/runbooks/secrets-migrations-lifecycle.md` and `ops/runbooks/security-operations.md` (FR-113, FR-116) — REOPENED — self-hosted service architecture reconciliation
