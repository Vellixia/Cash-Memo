...
- [x] T004 [P] Define fail-closed canonical runtime configuration for separate runtime/identity PostgreSQL connections, RustFS Primary/Secondary, pgBackRest, deletion ledger, OTLP, OpenAI, and Mailpit/Cloudflare email in `.env.example` and `apps/server/src/bootstrap/environment.schema.ts` (FR-083) — REOPENED — self-hosted service architecture reconciliation
- [x] T005 [P] Provide development bootstrap guidance compatible with the preserved external PostgreSQL, Mailpit/contract providers, future RustFS services, and shared OTLP without local Docker or duplicate shared services in `infra/containers/compose.yaml` and `scripts/local/bootstrap-https.mjs` — REOPENED — self-hosted service architecture reconciliation
...
- [x] T052 [P] [US1] Implement project-owned email port, generic verification/reset templates, Mailpit development adapter, Cloudflare Email Service production adapter, bounded failure mapping, and content-free delivery events in `apps/server/src/modules/identity/email.port.ts`, `apps/server/src/adapters/mailpit/mailpit-email.adapter.ts`, and `apps/server/src/adapters/cloudflare/cloudflare-email.adapter.ts` — REOPENED — self-hosted service architecture reconciliation
...
- [x] T125 [P] [US3] Create versioned multilingual synthetic detector corpus and rule-family precision/recall/adversarial tests in `packages/privacy-rules/tests/detector-v1.spec.ts` and `packages/privacy-rules/fixtures/corpus-v1.json` (FR-075, FR-076)
- [x] T126 [US3] Implement v1 ephemeral normalization and finite PAN/IBAN/secret/account/identity detector families without candidate/span persistence in `packages/privacy-rules/src/detector-v1.ts` (FR-075–FR-077)
- [x] T127 [P] [US3] Write provider-port contract tests for STT, extraction, validation, bounded retry, safe errors, consent, and provider-type isolation in `apps/server/tests/contract/assisted-provider-ports.contract.spec.ts` (FR-036, FR-049, FR-050, FR-080, FR-086)
- [x] T128 [US3] Define project-owned `SttPort`, `ExtractionPort`, provider-neutral result states, strict schemas, and deadlines in `apps/server/src/modules/assisted-capture/provider-ports.ts`
- [x] T129 [US3] Implement deterministic strict STT/extraction fakes and replacement toggles against those contracts in `apps/server/src/adapters/fakes/assisted-provider.adapters.ts`
- [x] T130 [P] [US3] Write typed-text extraction tests for relative anchor, byte-equivalent recovery, detector block, consent, ambiguity, invalid output, and no auto-confirm in `apps/server/tests/integration/text-extraction.spec.ts` (FR-019, FR-032, FR-034–FR-036, FR-048, FR-050)
- [x] T131 [US3] Implement typed natural-language extraction use case that persists only checked project-owned draft state in `apps/server/src/modules/assisted-capture/text-extraction.service.ts` (FR-032, FR-034–FR-037)
- [x] T132 [US3] Implement typed capture UI with adjacent prohibited-data guidance, explicit provider consent, editable uncertainty, and live-memory correction on block in `apps/web/src/features/capture/NaturalLanguageCapture.tsx` (FR-034, FR-050, FR-074)
- [x] T133 [P] [US3] Write MIME/magic/codec/size/measured-duration, permission, 60-second auto-stop, interruption, and spoofing tests in `apps/server/tests/integration/audio-admission.spec.ts` and `apps/web/tests/component/voice-recorder.spec.tsx` (FR-043, FR-046)
- [x] T134 [US3] Implement request-owned bounded-memory/encrypted-ephemeral audio lifecycle with content-free metadata and `try/finally` deletion in `apps/server/src/modules/assisted-capture/temporary-audio.service.ts` (FR-038, FR-044, FR-045)
- [x] T135 [US3] Implement one-minute expiry sweeper, startup directory cleanup, refusal-after-expiry, deletion retry/alert, and task-termination hook in `apps/server/src/modules/operations/audio-sweeper.ts` (FR-044, FR-045)
- [x] T136 [P] [US3] Write voice state-machine, lost-upload-response, incomplete transcript, STT/AI failure, and never-confirm-partial integration tests in `apps/server/tests/integration/voice-capture.spec.ts` (FR-033, FR-037, FR-038, FR-046)
- [x] T137 [US3] Implement voice capture start/upload/status/cancel orchestration and provider-neutral OpenAPI endpoints in `apps/server/src/modules/assisted-capture/voice-capture.service.ts` and `apps/server/src/modules/assisted-capture/voice-capture.controller.ts` (FR-033, FR-043–FR-046)
- [x] T138 [US3] Implement browser MediaRecorder flow with explicit start, provider notice, elapsed/remaining timer, 60-second hard stop, permission/interruption states, and no durable audio in `apps/web/src/features/capture/VoiceRecorder.tsx` (FR-043, FR-046, FR-050, FR-074)
- [x] T139 [US3] Run transcript detector before persistence/AI, label incomplete text, and persist allowed transcript only in recoverable draft in `apps/server/src/modules/assisted-capture/transcript.service.ts` (FR-037, FR-046, FR-075)
- [x] T140 [US3] Implement strict extraction-output validation for money/time/ownership/labels/extra fields/future values and correction states in `apps/server/src/modules/assisted-capture/extraction-validation.ts` (FR-034–FR-036, FR-080)
- [x] T141 [US3] Implement transcript and structured-draft review UI with editable every field and explicit inferred/uncertain/missing/contradictory states in `apps/web/src/features/capture/AssistedDraftReview.tsx` (FR-034)
- [x] T142 [US3] Enforce operation-specific STT/text/transcript-to-AI consent and disclose raw-voice-before-text-detection limitation in `apps/server/src/modules/assisted-capture/consent-policy.ts` and `apps/web/src/features/privacy/ProviderConsent.tsx` (FR-050)
- [x] T143 [P] [US3] Write repeated-confirmation, changed-fields-authority, stale-draft, and confirmed-memo-nonmutation tests in `apps/server/tests/integration/assisted-confirmation.spec.ts` (FR-039, FR-047)
- [x] T144 [US3] Implement separate idempotent explicit assisted-draft confirmation transaction that revalidates user-edited fields and schedules draft/derived cleanup in `apps/server/src/modules/assisted-capture/confirm-draft.service.ts` (FR-032, FR-039, FR-042, FR-047)
- [x] T145 [US3] Map timeout/rate-limit/refusal/invalid-schema/partial-provider states to content-free visible degradation without altering confirmed records in `apps/server/src/modules/assisted-capture/provider-failure-policy.ts` (FR-036–FR-039, FR-049)
- [x] T146 [P] [US3] Implement protected real-OpenAI STT adapter contract harness for pinned model/formats/errors with SDK logging disabled in `tests/providers/openai-stt.contract.spec.ts`
- [x] T147 [P] [US3] Implement protected real-OpenAI extraction adapter contract harness for pinned snapshot, `store:false`, strict schema, no tools/files/background, and invalid/ambiguous cases in `tests/providers/openai-extraction.contract.spec.ts`
- [x] T148 [US3] Prove success/cancel/unrecoverable-failure/one-hour-expiry/task-kill audio deletion with direct absence probes in `tests/operations/audio-lifecycle.spec.ts` (FR-044, FR-045, FR-103; SC-009)
- [x] T149 [US3] Implement outbound provider capture proxy that records field names/length classes only and verifies current-capture minimization in `packages/test-support/src/providers/payload-capture-proxy.ts` (FR-078, FR-086)
- [x] T150 [US3] Create `pnpm acceptance:us3` suite for every typed/voice success, ambiguity, invalid, interruption, privacy, deletion, edit, and confirmation scenario in `tests/acceptance/us3-assisted-capture.spec.ts`
- [x] T151 [US3] Invoke `pnpm acceptance:us3` with contract fakes and store clearly non-real-provider story evidence in `ops/evidence/stories/us3-integration.json`; keep real-provider closure open (FR-105, FR-118)
...
- [x] T152 [P] [US5] Define executable failure matrix for STT, AI, network, DB, auth, Cloudflare email, RustFS Primary/Secondary, pgBackRest, telemetry, worker, and calculation fault points in `tests/failure/failure-matrix.ts` (FR-031, FR-111) — REOPENED — self-hosted service architecture reconciliation
...
- [x] T167 [P] [US8] Implement the project-owned export storage port with a private S3-compatible RustFS Primary adapter, opaque keys, checksum/head verification, application-mediated streams, version deletion, bounded retry/ambiguity handling, and contract fake in `apps/server/src/adapters/rustfs/export-object-store.adapter.ts` (FR-094) — REOPENED — self-hosted service architecture reconciliation
...
- [x] T170 [US8] Test controlled-clock cancel/expiry/inaccessibility and every RustFS object version/delete-marker removal within 24 hours, including retry and ambiguous response behavior, in `tests/operations/export-lifecycle.spec.ts` (FR-094, FR-103; SC-020) — REOPENED — self-hosted service architecture reconciliation
...
- [x] T185 [P] Write RustFS Secondary deletion-ledger contract and conditional integration tests for durable writes, repository encryption posture, no raw identity/content, no lifecycle TTL, ambiguous responses, version checks, and privileged removal in `tests/providers/rustfs-deletion-ledger.contract.spec.ts` (FR-100) — REOPENED — self-hosted service architecture reconciliation
- [x] T186 Implement the S3-compatible RustFS Secondary deletion-suppression adapter with content-free keys/body/checksum/version verification and fail-closed ambiguity handling in `apps/server/src/adapters/rustfs/deletion-suppression.adapter.ts` (FR-100) — REOPENED — self-hosted service architecture reconciliation
...
- [x] T189 [P] Write full self-hosted lineage inventory contracts for pgBackRest backup sets/WAL, local repositories, Secondary RustFS, manual/operator/volume copies, replicas, and temporary restores in `tests/providers/backup-lineage-inventory.contract.spec.ts` (FR-100) — REOPENED — self-hosted service architecture reconciliation
- [x] T190 Implement authoritative pgBackRest/RustFS/local-copy/replica/restore-copy inventory adapter with complete/current, resurrection-capable, destroyed/expired, stale/incomplete, unavailable, and unverifiable states in `apps/server/src/adapters/backup/backup-lineage-inventory.adapter.ts` (FR-100) — REOPENED — self-hosted service architecture reconciliation
...
- [x] T192 Implement verifier-controlled suppression cleanup over the self-hosted lineage model, retaining/alerting/retrying on every failed, stale, incomplete, unavailable, or unverifiable proof in `apps/server/src/modules/deletion/suppression-cleanup.service.ts` (FR-100) — REOPENED — self-hosted service architecture reconciliation
- [x] T193 Add deployment/static policy forbidding untracked local/manual/operator/volume copies and unregistered replicas or restore copies, and requiring an independent production backup failure domain, in `infra/dokploy/policies/backup-copy-policy.json` (FR-100) — REOPENED — self-hosted service architecture reconciliation
- [x] T194 [P] Write pgBackRest isolated-restore tests for account-first then memo-token matching across key versions, expired sweepers, session revocation, neighboring-data survival, and release denial in `tests/operations/restore-reconciliation.spec.ts` (FR-028, FR-100, FR-115; SC-021) — REOPENED — self-hosted service architecture reconciliation
- [x] T195 Implement isolated pre-network pgBackRest PITR reconciliation command with exact account/memo suppression and mandatory verification checklist in `scripts/operations/restore-reconcile.mjs` (FR-100, FR-115) — REOPENED — self-hosted service architecture reconciliation
- [x] T196 Implement registered pgBackRest restore-copy lifecycle, lineage inventory, network-isolation guard, and verified destruction command in `scripts/operations/restore-copy-lifecycle.mjs` (FR-100) — REOPENED — self-hosted service architecture reconciliation
- [x] T197 Create production-safe pgBackRest PITR restore/release runbook with independent failure-domain requirement, owners, approvals, RPO/RTO, and content-safe evidence in `ops/runbooks/backup-restore.md` (FR-114, FR-115) — REOPENED — self-hosted service architecture reconciliation
- [x] T198 Create self-hosted suppression cleanup/key rotation/copy-policy-drift runbook with blocker retention and escalation in `ops/runbooks/deletion-suppression-cleanup.md` (FR-100) — REOPENED — self-hosted service architecture reconciliation
- [ ] T199 Build `pnpm test:operations` pgBackRest backup/WAL/PITR/restore/purge/cleanup orchestration using synthetic identities only in `tests/operations/operations-suite.ts` (FR-103, FR-114, FR-115) — REOPENED — self-hosted service architecture reconciliation
- [x] T200 Define quarterly isolated pgBackRest restore and cleanup drill automation, evidence schema, real restored-state checks, and pass/fail rules in `ops/drills/quarterly-restore.ts` and `ops/drills/suppression-cleanup.ts` (FR-114, FR-115; SC-021) — REOPENED — self-hosted service architecture reconciliation
...
- [x] T211 [P] Execute full endpoint/RLS/FK/object-store/audio/deletion IDOR matrix, missing/forged context, pool reset, and maintenance-role separation in `tests/security/cross-user-isolation.spec.ts` (FR-010, FR-060, FR-081, FR-082; SC-015)
...
**Purpose**: Repeatable Docker/Dokploy staging and production topology, deployment, injected secrets, shared telemetry, self-hosted storage/backups, external-input binding, and runbooks.

- [x] T219 [P] Define the reviewed self-hosted service/version contract, immutable input policy, Dokploy environment template, and drift-check schema in `infra/dokploy/service-versions.json`, `infra/dokploy/environment.example`, and `infra/dokploy/README.md` — REOPENED — self-hosted service architecture reconciliation
- [x] T220 [P] Define private Dokploy-network attachment, no-public-object-store posture, API-only ingress boundary, controlled egress requirements, and no duplicate shared services in `infra/dokploy/compose.yaml` and `infra/dokploy/network-policy.json` — REOPENED — self-hosted service architecture reconciliation
- [x] T221 [P] Define one immutable Cashmemo OCI image digest serving separate API/worker roles, non-root/read-only runtime, health checks, and safe restart/rollback behavior in `infra/dokploy/compose.yaml` (FR-116) — REOPENED — self-hosted service architecture reconciliation
- [x] T222 [P] Preserve the existing private PostgreSQL 18 service and volume, add non-destructive health/restart configuration guidance, and configure pgBackRest backup/WAL/PITR without inventing resource limits in `infra/dokploy/postgres-preservation.md` and `infra/pgbackrest/` (FR-100, FR-114) — REOPENED — self-hosted service architecture reconciliation
- [x] T223 [P] Define private versioned RustFS Primary export/evidence buckets and distinct RustFS Secondary pgBackRest/deletion-ledger namespaces, encryption requirements, zero raw-audio storage, and no suppression TTL in `infra/dokploy/compose.yaml` and `infra/dokploy/storage-policy.json` (FR-094, FR-100) — REOPENED — self-hosted service architecture reconciliation
- [x] T224 [P] Configure Mailpit development and Cloudflare Email Service production delivery selection with content-free status mapping and no external configuration mutation in `infra/dokploy/environment.example` and the email adapters — REOPENED — self-hosted service architecture reconciliation
- [x] T225 [P] Define existing-Infisical runtime secret injection and separated runtime/worker/migration/restore/deploy/break-glass capabilities without deploying another secret service in `infra/dokploy/secrets-policy.md` (FR-083) — REOPENED — self-hosted service architecture reconciliation
- [x] T226 [P] Bind Cashmemo OTLP export to the existing shared collector/OpenObserve using allowlist-only telemetry, without deploying duplicate observability services, in `infra/dokploy/observability.md` (FR-110, FR-112) — REOPENED — self-hosted service architecture reconciliation
- [x] T227 Integrate pgBackRest backup/PITR, copy-policy drift, inventory-failure, missed-drill, RPO/RTO alert contracts, and independent-failure-domain production checks in `infra/dokploy/backup-alert-policy.json` (FR-100, FR-114) — REOPENED — self-hosted service architecture reconciliation
- [x] T228 Implement a Dokploy-compatible one-shot migration role using the existing advisory lock/schema compatibility/evidence command in `infra/dokploy/compose.migration.yaml` and `scripts/db/migrate-production.mjs` (FR-116) — REOPENED — self-hosted service architecture reconciliation
- [x] T229 Harden the shared API/worker OCI image with pinned base digest inputs, minimal packages/capabilities, non-root/read-only runtime, role-specific health probes, and SBOM attachment in `infra/containers/Dockerfile` and `infra/containers/docker-bake.hcl` (FR-116) — REOPENED — self-hosted service architecture reconciliation
- [x] T230 Implement GitHub Actions build/test/scan/SBOM/publish workflow producing an immutable digest and protected Dokploy handoff artifact; deployment remains a separate approved pass in `.github/workflows/deploy.yml` (FR-116, FR-117) — REOPENED — self-hosted service architecture reconciliation
- [x] T231 Implement and rehearse Dokploy rollback plus expand/contract safe-forward decision paths using the immutable image digest in `scripts/deploy/rollback-or-safe-forward.mjs` and `ops/runbooks/deployment-rollback.md` (FR-116) — REOPENED — self-hosted service architecture reconciliation
- [ ] T232 In a separate Dokploy MCP deployment pass, preserve the existing PostgreSQL service/volume, deploy API + Worker from one immutable digest, deploy pinned RustFS Primary and same-host Secondary Development, deploy/configure pgBackRest, connect shared Infisical and OTLP, run drift checks, and write honest development evidence to `ops/evidence/infrastructure/dokploy-development-bootstrap.json` (FR-105)
- [ ] T233 Bind—not fabricate—approved Dokploy project/environment/resource IDs, application hostname/TLS boundary, immutable image digest, shared-service endpoints, independent production Secondary identity, and protected workflow identity in `infra/dokploy/production-inputs.example` and `ops/evidence/external/dokploy-environment.json`; fail closed while missing
...
- [ ] T237 Obtain and verify—not fabricate—Cloudflare Email Service production/domain authorization, API-token scope, sender readiness, and controlled-inbox delivery in protected reference `ops/evidence/external/cloudflare-email-production-approval.json`; keep signup release blocked if absent
...
- [x] T240 Create and rehearse core journal, STT/AI, Cloudflare email, PostgreSQL/pgBackRest, RustFS, shared telemetry, and provider-config-drift outage runbooks in `ops/runbooks/core-journal-outage.md`, `ops/runbooks/provider-outages.md`, and `ops/runbooks/self-hosted-service-outages.md` (FR-113) — REOPENED — self-hosted service architecture reconciliation
- [x] T241 Create and rehearse injected-secrets, migration, Dokploy deployment, audio, lifecycle backlog, export exposure, RustFS/backup, rate-abuse, and cross-user incident runbooks in `ops/runbooks/secrets-migrations-lifecycle.md` and `ops/runbooks/security-operations.md` (FR-113, FR-116) — REOPENED — self-hosted service architecture reconciliation
...
- [ ] T261 Run real-service Better Auth + deployed PostgreSQL signup/login/reset/restoration/revocation/expiry/30-day policy and two-account isolation suite; record safe evidence in `ops/evidence/real-services/better-auth-postgresql.json` (FR-001–FR-003, FR-105; SC-003, SC-004, SC-015)
- [ ] T262 Run real-service Cloudflare Email verification/reset delivery, generic copy, permanent-bounce mapping, timeout/retry, and telemetry exclusion against controlled inboxes; record safe evidence in `ops/evidence/real-services/cloudflare-email.json` (FR-001, FR-105)
...
- [ ] T265 Run real-service pinned RustFS Primary export isolation, encryption posture, head/checksum, application stream, expiry, version deletion, ambiguity, and completeness/reproduction suite; record safe evidence in `ops/evidence/real-services/rustfs-primary-export.json` (FR-091–FR-094, FR-105; SC-019, SC-020)
- [ ] T266 Run real-service Dokploy immutable API/worker deployment, rollback, injected secrets, read-only/non-root runtime, task-kill audio cleanup, and one-shot migration suite; record safe evidence in `ops/evidence/real-services/dokploy-runtime.json` (FR-105, FR-116)
- [ ] T267 Run real-service Cashmemo→shared OTel Collector→OpenObserve allowlist/canary/alert/core-provider-separation suite; record safe evidence in `ops/evidence/real-services/observability.json` (FR-078, FR-079, FR-105, FR-112; SC-016)
- [ ] T268 Run real-service isolated pgBackRest target-time PITR drill from an independent-failure-domain repository: RPO≤24h/RTO≤8h, inspect restored state, account-first + memo suppression, valid neighbors survive, no network before checks, all cleanup blockers retain/alert/retry, restore copy destroyed before token removal; record safe evidence in `ops/evidence/operations/sc021-restore-drill.json` (FR-100, FR-114, FR-115; SC-021)
...
|---|---|---|
| Dokploy resource IDs, hostname/TLS boundary, immutable digest, shared-service endpoints, independent Secondary | T233 | `ops/evidence/external/dokploy-environment.json`; missing blocks staging/production promotion |
...
| Cloudflare Email production/domain authorization | T237 | protected `ops/evidence/external/cloudflare-email-production-approval.json`; absent blocks real signup acceptance |
