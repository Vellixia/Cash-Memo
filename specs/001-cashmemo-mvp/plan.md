...
**Storage**: PostgreSQL 18/RDS Multi-AZ for product and job state; browser IndexedDB for the user's same-device draft replica; encrypted ECS task-local ephemeral space only when an STT adapter requires a temporary file; private KMS-encrypted S3 for expiring export packages, content-safe evidence, and deletion-suppression records  
...
**Target Platform**: AWS `ap-southeast-1`; ECS Fargate behind ALB; current and previous major Chrome, Edge, Firefox, and Safari on supported desktop/mobile OSes  
...
project-owned ports
                 ↑
PostgreSQL, OpenAI, SES, S3, and telemetry adapters
```

Modules are `identity`, `onboarding`, `memo`, `draft`, `assisted-capture`, `labels`, `history`, `reporting`, `export`, `deletion`, `privacy`, and `operations`. A module may call another module only through an application port. Domain modules import no NestJS, Drizzle, Better Auth, OpenAI, AWS, or browser types.
...
- Email verification is required before a journal session is issued. AWS SES sends generic, short-lived verification and reset links; email bodies contain no financial content. Local development uses Mailpit.
...
- Export objects live in a private KMS-encrypted S3 bucket under opaque account-scoped keys. A recent-authenticated endpoint issues a single-account, five-minute signed download; the package expires within 24 hours, and cancellation/expiry queues deletion within 24 hours.
...
- Before an irreversible purge deletes live content, the deletion owner durably writes a scope-specific ledger record outside resurrection-capable RDS data. Its identity is `deletion_token = HMAC-SHA-256(suppression_key, entity_type || ":" || immutable_entity_id)`, with `entity_type` at minimum `money_memo|account`. The ledger stores only token, entity type, suppression-key version, purge time, `removal_not_before_at`, policy version, and content-free verification state—never raw account/memo ID, email, financial value, journal metadata, or content derivative. Failure to write the token leaves the entity inaccessible in `purging` and blocks irreversible live deletion.
- `removal_not_before_at` is purge time plus 42 days (35-day automated-backup maximum plus seven-day margin), not an expiry or deletion command. After that time, a privileged verifier must prove every resurrection-capable automated-backup recovery window is closed and no manual/final/copied snapshot, retained automated backup, AWS Backup recovery point, cross-region/replicated backup, or active isolated restore copy from the database lineage can restore pre-purge state. Only then may it delete the token. Verification failure/unavailability retains the token, raises an alert, and retries; cleanup remains incomplete.
- Feature 001 infrastructure prohibits manual/final database snapshots, retained automated backups after database deletion, AWS Backup recovery points, snapshot sharing/copying, and cross-region automated-backup replication through least-privilege IAM and policy-as-code. CloudTrail/EventBridge/AWS Config-style inventory detects violations. Isolated restore copies are the sole planned extra resurrection-capable copy; every copy is tagged/inventoried, network-isolated, and must be verified destroyed before suppression cleanup. S3 lifecycle may provide a conservative retention floor but is never final deletion authority.
...
- OpenTofu provisions VPC, private subnets, ALB, ECS/Fargate, ECR, RDS PostgreSQL Multi-AZ, S3/KMS, SES identities, Secrets Manager, CloudWatch alarms, IAM, and an OpenTelemetry collector sidecar. Production is `ap-southeast-1`; staging mirrors topology at smaller capacity.
- GitHub Actions builds a pinned, non-root, read-only-root-filesystem OCI image; runs gates in constitutional order; generates an SBOM; scans dependencies/image/secrets; applies migrations as a one-shot ECS task; verifies migration state; deploys with ECS circuit breaker; and runs synthetic acceptance before promotion. Backward-compatible expand/contract migrations permit rollback; otherwise safe-forward recovery is mandatory and rehearsed.
- Secrets Manager supplies runtime secrets through task IAM. No production secret enters source, image, build log, test evidence, or browser bundle.
- OpenTelemetry emits allowlisted signals to CloudWatch/X-Ray-compatible backends. Alarms cover core availability/latency/error rate, provider health separately, DB saturation, worker backlog/lease age, deletion/audio/export expiry, SES failures, and backup/restore status.
- Daily RDS backups/PITR target RPO ≤24 hours. Quarterly isolated restore drills target RTO ≤8 hours and verify RLS isolation, exact account- and memo-token suppression, money/currency integrity, deterministic totals, export isolation, restore-copy destruction, and safe release. S3 export/evidence buckets use versioning and scoped lifecycle policies; suppression-ledger removal is verifier-controlled rather than TTL-controlled, and raw audio is absent from all backups.
...
└── server/
    ...
    │   ├── adapters/{http,postgres,openai,aws,telemetry}/
...
├── providers/          # real OpenAI, SES, S3, and RDS contract/smoke tests
