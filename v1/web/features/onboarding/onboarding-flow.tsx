...
export function OnboardingFlow() {
  ...
  async function savePreferences() {
    setStatus(undefined);
    if (!defaultCurrency) {
      setStatus({ kind: "error", text: "Choose a currency." });
      return;
    }
    try {
      await onboarding.savePreferences({ timezone, default_currency_code: defaultCurrency });
  ...
  async function seedCategories() {
    setStatus(undefined);
    try {
      await onboarding.seedCategories();
      setStatus({ kind: "success", text: "Starter categories ready." });
    } catch (error) {
      setStatus({ kind: "error", text: errorText(error) });
    }
  }
  ...
  return (
    <main className="public-page">
      <section className="onboarding-shell" aria-labelledby="onboarding-title">
        ...
        <h1 id="onboarding-title">Make Cashmemo useful</h1>
        <p className="muted">
          You can leave and return anytime. Progress below comes from your saved account state.
        </p>
        {onboarding.step === "timezone" ? (
          <div className="dialog onboarding-step">
            <h2>Confirm your timezone</h2>
            <p>
              We use it for local dates, months, and recurring transactions. Choose an IANA
              timezone; the server validates it.
            </p>
            <FormField label="Reporting timezone" htmlFor="onboarding-timezone">
              <input
                id="onboarding-timezone"
                className="input"
                list="cashmemo-timezones"
                value={timezone}
                onChange={(event) => {
                ...
                autoComplete="off"
              />
              <datalist id="cashmemo-timezones">
                {timezoneOptions.map((option) => (
                  <option key={option} value={option} />
                ))}
              </datalist>
            </FormField>
            <FormField label="Default currency" htmlFor="onboarding-currency">
              <select
            ...
            <Button
              ...
              disabled={onboarding.isMutating || !timezone.trim() || !defaultCurrency}
            >
        ...
        {onboarding.step === "currency" ? (
          <div className="dialog onboarding-step">
            ...
            <p>This only preselects new wallets. Cashmemo never converts between currencies.</p>
            <FormField label="Default currency" htmlFor="onboarding-currency">
              <select
                id="onboarding-currency"
                className="input"
            ...
            <Button
              type="button"
              onClick={() => void savePreferences()}
              disabled={onboarding.isMutating || currencies.isError || !defaultCurrency}
            >
              Save currency
            </Button>
          </div>
        ...
        {onboarding.step === "categories" ? (
          <div className="dialog onboarding-step">
            <h2>Start with useful categories</h2>
            <p>
              Starter categories are safe to seed repeatedly. Your own categories stay alongside
              them with the same controls.
            </p>
            <Button
              type="button"
            ...
            >
              {onboarding.isMutating ? "Preparing…" : "Add starter categories"}
            </Button>
          </div>
        ) : null}
